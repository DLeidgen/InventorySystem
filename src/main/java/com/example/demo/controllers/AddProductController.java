...
// make the add and remove buttons work
 ...
 //        return "confirmationassocpart";
    ...
    // Buy Guitar
    @GetMapping("/buyGuitar")
    public String buyGuitar(@RequestParam("productID") int theId, Model theModel) {
        ProductService productService = context.getBean(ProductServiceImpl.class);
        Product guitar = productService.findById(theId);
        int invCount = guitar.getInv();

        if (invCount == 0) {
            theModel.addAttribute("message", "There are no guitars to buy!");
            return "noinventory";
        }
        else {
            guitar.setInv(invCount - 1);
            productService.save(guitar);
            theModel.addAttribute("product", guitar);
            theModel.addAttribute("message", "You bought a guitar!");
            return "confirmguitar";
        }
    }
